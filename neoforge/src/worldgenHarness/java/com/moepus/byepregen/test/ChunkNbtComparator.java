...
final class ChunkNbtComparator {
    ...
    private static String compareCompounds(CompoundTag expected, CompoundTag actual, String path) {
        ...
        if (!keys.equals(new TreeSet<>(actual.keySet()))) {
            return path + ": expected keys=" + keys + ", actual keys=" + new TreeSet<>(actual.keySet());
        }
        for (String key : keys) {
    ...
    private static String comparePalettedData(
        ...
        long[] expectedData = expected.getLongArray("data").orElseGet(() -> new long[0]);
        long[] actualData = actual.getLongArray("data").orElseGet(() -> new long[0]);
    ...
    private static String comparePaletteShape(CompoundTag expected, CompoundTag actual, String path) {
        Set<String> expectedKeys = new TreeSet<>(expected.keySet());
        Set<String> actualKeys = new TreeSet<>(actual.keySet());
        ...
        if (expectedData && !(expected.get("data") instanceof LongArrayTag)
                || actualData && !(actual.get("data") instanceof LongArrayTag)) {
