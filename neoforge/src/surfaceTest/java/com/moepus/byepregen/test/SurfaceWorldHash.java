...
public final class SurfaceWorldHash {
    private static final RegionStorageInfo REGION_INFO = RegionStorageInfos.overworld("surface-world-hash");
    ...
    private static ChunkHashes hashChunk(CompoundTag chunk) throws IOException {
        CompoundTag heightmaps = chunk.getCompound("Heightmaps").orElseGet(CompoundTag::new);
        return new ChunkHashes(
                hashTag(blockStates(chunk)),
                hashTag(heightmaps),
                heightmapEntries(heightmaps),
                hashTag(chunk.getList("PostProcessing").orElseGet(ListTag::new)),
                chunk.getString("Status").orElse("")
        );
    }
    ...
    private static ListTag blockStates(CompoundTag chunk) {
        ...
        ListTag sections = chunk.getList("sections").orElseGet(ListTag::new);
        for (int index = 0; index < sections.size(); index++) {
            CompoundTag source = sections.getCompound(index).orElseGet(CompoundTag::new);
            if (!source.contains("block_states")) {
                continue;
            }
            CompoundTag section = new CompoundTag();
            section.putByte("Y", source.getByte("Y").orElse((byte) 0));
            section.put("block_states", source.getCompound("block_states").orElseThrow().copy());
            result.add(section);
        }
        return result;
    }
    ...
    private static void writeCanonical(Tag tag, DataOutputStream output) throws IOException {
        ...
        if (tag instanceof CompoundTag compound) {
            ArrayList<String> keys = new ArrayList<>(compound.keySet());
