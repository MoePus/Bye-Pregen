...
/**
 * 26.3 moved the light-colour helpers from LevelRenderer to {@link LightCoordsUtil}; the overwrites stay
 * on the same logic so every caller of those helpers reads light through YA storage.
 */
@MixinGate(feature = MixinFeature.YA_LIGHT)
@Mixin(value = LightCoordsUtil.class, priority = 900)
public abstract class LightCoordsUtilYALightMixin {
    /**
     * @author
     * @reason
     */
    ...
    public static int getLightCoords(
        ...
        if (blockGetter instanceof Level level) {
            ...
            light = brightnessGetter.packedBrightness(blockGetter, pos);
        }
        if (state.isAir()) {
            return light;
        }
