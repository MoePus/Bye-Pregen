...
public final class NetworkWriter {
    ...
    private static int uniformSerializedSize(int rawId) {
        return 1 + FriendlyByteBuf.getVarIntSize(rawId) + FriendlyByteBuf.getVarIntSize(0);
    }
    ...
    private static void write(
        ...
        buffer.writeVarInt(scratch.beginPack(bits));
    ...
    private static int serializedSize(SerializationScratch scratch) {
        ...
        return 1 + paletteSerializedSize(scratch, bits, useGlobalPalette(bits), paletteSize)
                + FriendlyByteBuf.getVarIntSize(packedLength)
    ...
    private static int paletteSerializedSize(
        ...
        if (bits == 0) {
            return FriendlyByteBuf.getVarIntSize(scratch.paletteRawId(0));
        }

        int size = FriendlyByteBuf.getVarIntSize(paletteSize);
        for (int i = 0; i < paletteSize; ++i) {
            size += FriendlyByteBuf.getVarIntSize(scratch.paletteRawId(i));
        }
        return size;
    }
}
