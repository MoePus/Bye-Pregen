...
final class LightChunk {
    ...
    private void readSection(CompoundTag section) {
        ...
        if (section.contains("block_states", 10)) {
            this.blocks.put(sectionY, SectionBlocks.from(section.getCompound("block_states")));
        }
        for (String lightKey : LIGHT_KEYS) {
            if (section.contains(lightKey, 7)) {
                this.lights.put(new LightSectionKey(sectionY, lightKey), section.getByteArray(lightKey));
            }
        }
    }
